...
function startup() 
{
    ...
    gID('optFemale').disabled = false;
    gID('optMale').disabled = false;
...
function func_femaleCheck()
{
	if (gID('optFemale').checked == true)
	{
		gID('optMale').checked = false;
	}
}

function func_maleCheck()
{
	if (gID('optMale').checked == true)
	{
		gID('optFemale').checked = false;
	}
}
...
function init()
{
    ...
    if (int_Total < 2) 
    {
        alert("Please make a selection. Not enough character selected.");
        return;
    } 
    else 
    {
...
function fnc_ShowData() 
{
    ...
    if (int_Status == 2) 
    {
        ...
        for (i = 0; i < ary_TempData.length; i++) 
        {
            ...
            new_cell = new_row.insertCell(new_row.childNodes.length);
            sC(new_cell, 'resTableL');
            var textIntResult = cE('div');
            
            //textIntResult.appendChild(cT(int_Result));
            if (i < int_numberImageResult)
            {
                textIntResult.className = "bigNumber";
            }
            textIntResult.innerHTML = int_Result;
            new_cell.appendChild(textIntResult);
            ...
            new_cell = new_row.insertCell(new_row.childNodes.length);
            if (i == 0)
                sC(new_cell, 'resTableBigImage');
            else if (i < int_numberImageResult)
                sC(new_cell, 'resTableImage');
            else
                sC(new_cell, 'resTableR');

            var bln_imgFlag = false;
            if ((int_ResultImg != 0) && (i < int_numberImageResult)) 
            {
                var new_img = cE('img');
                var obj_TempData = ary_TempData[ary_SortData[0][i]];
                
                if (i == 0)
                    new_img.className = "bigImage";
            ...
            var cutoff = int_numberImageResult - 1;
            var rowIdPlusOne = rowId + 1;
            if ((rowId >= 0 && rowIdPlusOne < ary_TempData.length) && 
                (rowId == 0 || (rowId < cutoff && rowIdPlusOne % 2 == 1) ||
                (rowId >= cutoff && (rowId == cutoff || (rowId - cutoff) % maxRows == 0))))
            {
                tbl_Result = cE('table');
                tbl_Result.classList.add('resTable');
    ...
    else 
    {
        for (i=0; i<2; i++) 
        {
            ...
            var equal_Item = ary_SortData[(i == 0)  ? int_LeftList : int_RightList][(i == 0)  ? int_LeftID : int_RightID];
            var total_EqualItem = 1;
            while (ary_EqualData[equal_Item] != -1)
            {
                equal_Item = ary_EqualData[equal_Item];
                total_EqualItem++;
            }
            
            var random_EqualItem = Math.floor(Math.random() * (total_EqualItem + 1));
            equal_Item = ary_SortData[(i == 0)  ? int_LeftList : int_RightList][(i == 0)  ? int_LeftID : int_RightID];
            while (random_EqualItem > 0 && ary_EqualData[equal_Item] != -1)
            {
                random_EqualItem--;
                equal_Item = ary_EqualData[equal_Item];
            }
            
            var obj_TempData = ary_TempData[equal_Item];
            ...
            var obj_titItem = gID((i == 0) ? "titLeft" : "titRight");
            for (j=0; j<ary_TitleData.length; j++)
            {
                if (obj_TempData[2][j] == 1)
                {
                    obj_titItem.innerHTML = ary_TitleData[j];
                    break;
                }
            }
        }
